# AGE LENGTH KEY FOR WSH FEMALES

# otolith ages used (necessary for sex division)
# data has two columns
# one for length and one for age 
# many NAs in the dataset for the ones that do not have an age
...
# Clean data
female_wsh_key = data %>%
  filter(Species=="WSH", Sex=="Female") %>%
  rename(TL = `TL (mm)`, Age = `O FINAL AGE`) %>%
  select(TL, Age) %>%
  subset(TL>=350)
# error if you have fish from length sample with less than min length of fish from age sample
# min age sample length is 350
# only selected lengths >= 350


WSH.age <- subset(female_wsh_key, !is.na(Age)) # gets fish without NAs in age variable
...
WSH.len <- subset(female_wsh_key, is.na(Age)) #gets fish with NAs in age variable (no '!')
...
WSH.age1 <- lencat(~TL, data = WSH.age, startcat = 350, w=10) #categorized fish by size bin
#startcat=350 because my min length was 353
